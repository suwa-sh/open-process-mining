...
def execute_analysis(
    ...
    lead_time_stats = calculate_lead_time_statistics(process_type, filter_mode, date_from, date_to)
...
def get_preview(
    ...
    return {
        ...
        "date_range": {
            "min": result['min_date'].isoformat() if pd.notna(result['min_date']) else None,
            "max": result['max_date'].isoformat() if pd.notna(result['max_date']) else None
        },
        "filter_applied": {
            "mode": filter_mode,
            "date_from": date_from,
...
def calculate_lead_time_statistics(
    ...
    case_lead_times['lead_time'] = (case_lead_times['max'] - case_lead_times['min']).dt.total_seconds() / 3600
...
def _calculate_happy_path_lead_time(event_log: list[EventLog]) -> Dict[str, Any]:
    ...
    case_paths = df.groupby('case_id')['activity'].apply(lambda x: tuple(x.tolist())).reset_index()
