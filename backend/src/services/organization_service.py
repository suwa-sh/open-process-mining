...
def analyze_handover(
    ...
    for case_id in df['case_id'].unique():
        ...
        for i in range(len(case_df) - 1):
            ...
            if current_resource != next_resource and pd.notna(current_resource) and pd.notna(next_resource):
                waiting_time_hours = (next_row['timestamp'] - current_row['timestamp']).total_seconds() / 3600
                handovers[(current_resource, next_resource)]["count"] += 1
                handovers[(current_resource, next_resource)]["total_waiting_time"] += waiting_time_hours
    ...
    for _, row in unique_resources.iterrows():
        ...
        if pd.notna(resource_id):
            nodes.append({
                "id": resource_id,
                "label": row[resource_name_col] if pd.notna(row[resource_name_col]) else resource_id,
    ...
    for (source, target), data in handovers.items():
        avg_waiting_time = data["total_waiting_time"] / data["count"] if data["count"] > 0 else 0.0
        edges.append({
            "source": source,
            "target": target,
...
def analyze_workload(
    ...
    workload_stats = df.groupby([resource_id_col, resource_name_col]).agg(
    ...
    for _, row in workload_stats.iterrows():
        if pd.notna(row[resource_id_col]):
            workload.append({
                "resource_id": row[resource_id_col],
                "resource_name": row[resource_name_col] if pd.notna(row[resource_name_col]) else row[resource_id_col],
...
def analyze_performance(
    ...
    for case_id in df['case_id'].unique():
        ...
        for i in range(len(case_df) - 1):
            ...
            duration_hours = (next_row['timestamp'] - current_row['timestamp']).total_seconds() / 3600
    ...
    performance_stats = duration_df.groupby(['resource_id', 'resource_name']).agg(
    ...
    for _, row in performance_stats.iterrows():
        performance.append({
            ...
            "resource_name": row['resource_name'] if pd.notna(row['resource_name']) else row['resource_id'],
...
def create_organization_analysis(
    ...
    handover_data = analyze_handover(process_type, aggregation_level, filter_mode, date_from, date_to)
    workload_data = analyze_workload(process_type, aggregation_level, filter_mode, date_from, date_to)
    performance_data = analyze_performance(process_type, aggregation_level, filter_mode, date_from, date_to)
...
def get_organization_analyses(process_type: Optional[str] = None) -> List[Dict[str, Any]]:
    """
