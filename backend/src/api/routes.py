...
def get_analysis_by_id(analysis_id: UUID, db: Session = Depends(get_db)):
    ...
    analysis = db.query(AnalysisResultORM).filter(AnalysisResultORM.analysis_id == analysis_id).first()
...
def compare_analyses(before: UUID, after: UUID, db: Session = Depends(get_db)):
    ...
    """
    # Get both analyses
    before_analysis = db.query(AnalysisResultORM).filter(AnalysisResultORM.analysis_id == before).first()
    after_analysis = db.query(AnalysisResultORM).filter(AnalysisResultORM.analysis_id == after).first()

    if not before_analysis:
        raise HTTPException(status_code=404, detail=f"Before analysis {before} not found")
...
def calculate_comparison(before: Dict, after: Dict) -> Dict:
    ...
    before_edges = {(edge['source'], edge['target']): edge for edge in before.get('edges', [])}
    after_edges = {(edge['source'], edge['target']): edge for edge in after.get('edges', [])}
