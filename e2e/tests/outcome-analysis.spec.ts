...
test.describe('Outcome Analysis', () => {
  ...
  test('should display outcome analysis list page', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("📈 プロセス分析")')).toBeVisible();
    await expect(page.locator('button:has-text("🏢 組織分析")')).toBeVisible();
    await expect(page.locator('button:has-text("+ 新規分析を作成")')).toBeVisible();
  });
  ...
  test('should create path outcome analysis', async ({ page }) => {
    ...
    // Wait for modal
    await expect(page.locator('header:has-text("新規成果分析作成")')).toBeVisible();

    // Fill in fields
    const analysisName = `E2E Path Outcome ${Date.now()}`;
    await page.fill('input[placeholder="例: 受注金額分析_2025-10"]', analysisName);

    // Select process type
    const processTypeSelect = page.locator('label:has-text("プロセスタイプ")').locator('..').locator('select');
    await processTypeSelect.selectOption('order-delivery');

    // Wait for metrics to load (wait for select to have options)
    const metricSelect = page.locator('label:has-text("メトリック")').locator('..').locator('select');
    ...
    const analysisTypeSelect = page.locator('label:has-text("分析タイプ")').locator('..').locator('select');
    ...
    await expect(page.locator('text=プロセス: order-delivery')).toBeVisible({ timeout: 10000 });
    await expect(page.locator('button:has-text("← 成果分析一覧に戻る")')).toBeVisible();
  });
  ...
  test('should switch display mode in path outcome', async ({ page }) => {
    ...
    await page.click('button:has-text("+ 新規分析を作成")');
    await page.fill('input[placeholder="例: 受注金額分析_2025-10"]', `E2E Mode ${Date.now()}`);
    const processTypeSelect = page.locator('label:has-text("プロセスタイプ")').locator('..').locator('select');
    await processTypeSelect.selectOption('order-delivery');
    const metricSelect = page.locator('label:has-text("メトリック")').locator('..').locator('select');
    await metricSelect.waitFor({ state: 'attached', timeout: 5000 });
  ...
  test('should navigate back to list', async ({ page }) => {
    // Create analysis
    await page.click('button:has-text("+ 新規分析を作成")');
    await page.fill('input[placeholder="例: 受注金額分析_2025-10"]', `E2E Nav ${Date.now()}`);
    const processTypeSelect = page.locator('label:has-text("プロセスタイプ")').locator('..').locator('select');
    await processTypeSelect.selectOption('order-delivery');
    const metricSelect = page.locator('label:has-text("メトリック")').locator('..').locator('select');
    await metricSelect.waitFor({ state: 'attached', timeout: 5000 });
