...
test.describe('Organization Analysis', () => {
  ...
  test('should display organization analysis list page', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("+ 新規組織分析を作成")')).toBeVisible();
  });
  ...
  test('should create new organization analysis', async ({ page }) => {
    ...
    // Wait for modal
    await expect(page.locator('header:has-text("新規組織分析を作成")')).toBeVisible();

    // Fill in analysis name
    const analysisName = `E2E Org Test ${Date.now()}`;
    await page.fill('input[placeholder="例: employee-onboarding_組織分析_2025-10-04"]', analysisName);
    ...
    await expect(page).toHaveURL(/\/organization\/[a-f0-9-]+/, { timeout: 30000 });

    // Verify analysis detail page (wait for tabs to appear)
    await expect(page.locator('text=🔄 ハンドオーバー分析')).toBeVisible({ timeout: 10000 });
  ...
  test('should switch aggregation level', async ({ page }) => {
    ...
    await page.click('button:has-text("+ 新規組織分析を作成")');
    await page.fill('input[placeholder="例: employee-onboarding_組織分析_2025-10-04"]', `E2E Agg Test ${Date.now()}`);
    const processTypeSelect = page.locator('select').first();
    await processTypeSelect.selectOption('order-delivery');
    await page.locator('label:has-text("すべての期間を含める")').click();
    await page.click('button:has-text("分析を実行")');
  ...
  test('should navigate back to list', async ({ page }) => {
    // Create analysis
    await page.click('button:has-text("+ 新規組織分析を作成")');
    await page.fill('input[placeholder="例: employee-onboarding_組織分析_2025-10-04"]', `E2E Nav Test ${Date.now()}`);
    const processTypeSelect = page.locator('select').first();
    await processTypeSelect.selectOption('order-delivery');
    await page.locator('label:has-text("すべての期間を含める")').click();
    await page.click('button:has-text("分析を実行")');
