...
import { createAnalysis, getAnalysisPreview, getProcessTypes, getLeadTimeStats } from '../api/client';
...
const CreateAnalysisModal: React.FC<CreateAnalysisModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const [leadTimeStats, setLeadTimeStats] = useState<LeadTimeStats | null>(null);
              ...
              {isLoadingPreview ? (
                ...
                <VStack align="start" spacing={1} fontSize="sm">
                  ...
                  {leadTimeStats && leadTimeStats.lead_time_hours.median !== null && (
                    <>
                      <Text fontWeight="bold" mt={2}>リードタイム（開始〜終了）:</Text>
                      <Text ml={2}>
                      ...
                      {leadTimeStats.happy_path && leadTimeStats.happy_path.case_count > 0 && (
                        <>
                          <Text fontWeight="bold" mt={2}>✨ ハッピーパス:</Text>
                          ...
                          <Text ml={2}>
                            中央値: {leadTimeStats.happy_path.lead_time_hours.median?.toFixed(1)}時間
                          </Text>
                        </>
                      )}
                    </>
                  ...
                  {filterMode !== 'all' && (
                    <>
                      <Text mt={2}>
                        ケース{filterMode === 'case_start' ? '開始' : '完了'}期間:{' '}
                        {dateFrom} 〜 {dateTo}
                      </Text>
                      {preview.date_range.min && preview.date_range.max && (
                        <Text>
                          実際のイベント期間: {new Date(preview.date_range.min).toLocaleDateString('ja-JP')}{' '}
                          〜 {new Date(preview.date_range.max).toLocaleDateString('ja-JP')}
                        </Text>
                      )}
                    </>
                  )}
